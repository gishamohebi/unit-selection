...
def user_pass(user_id, password):
    """
    this function returns hashed of the inputs
    """
    user_id = user_id.encode()
    password = password.encode()
    return [hashlib.sha256(user_id).hexdigest(), hashlib.sha256(password).hexdigest()]


class RegisterStudent:
    """
    This class returns an object which is valid to add to 9920.csv or 9921.csv file
    """
    ...
    def write_to_file(self):
        ...
        if file_name is not None:
            my_passwords = read_data_in_file(f"{file_name}/{file_name}.csv", "student_code")
            my_id = read_data_in_file(f"{file_name}/{file_name}.csv", "id")
            if self.student_id not in my_id:
                if self.student_code not in my_passwords:
                    info = self.__dict__
                    file_writing(f"{file_name}/{file_name}.csv", info)
                    # creat hash of str
                    ...
                    writing_up_file(self.student_id, self.student_code, f"{file_name}/student_UP.csv")
            ...
            logging.warning("This is a program for id which starts with 9920 and 9921", exc_info=True)
            info = self.__dict__
            with open('others.json', "a") as myfile:  # writing json file
                json.dump(info, myfile)
            return (f"This app is for id which starts with 9920 and 9921\n"
    ...
    def important_units(cls, term, student_code):
        ...
        if term == 1:
            ...
            term = str(term)
            file_name = find_file_name(student_code)
            if file_name is not None:
                file_name = f"{file_name}/{file_name}_important.csv"
            else:
                print("This app is for id which starts with 9920 and 9921")
                logging.warning("This is a program for id which starts with 9920 and 9921", exc_info=True)
                not_passed_units = " "
